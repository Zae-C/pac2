...
int wallCollide(int x, int y, int dir, int level[31][28]) {
	...
	// Check along the far right side of the sprite, plus 4 (the speed we’re moving)
		new_x1 = x + 2 + PACSIZE * 2; //multiplying by 2 since PACSIZE is the radius
		new_x2 = x + 2 + PACSIZE * 2;
		new_x3 = x + 2 + PACSIZE * 2;
		...
		//Check at three points along that edge
		new_y1 = y + 2 + PACSIZE * 2;
		new_y2 = y + 2 + PACSIZE * 2;
		new_y3 = y + 2 + PACSIZE * 2;
...
void ghost::chase1(int x, int y, int level[31][28]) {
